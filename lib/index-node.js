...
SockJS = (function(){
              ...
              _window.WebSocket = require('faye-websocket').Client;
              _window.console = console;
              _window.attachEvent = _document.attachEvent = function() {};
    ...
    that.xhr.onreadystatechange = function() {
        if (that.xhr) {
            ...
            case 4:
				...
				//support for cookies add by zhh
            	var cookie = null;
            	if (status != 503) {
            		x.getResponseHeader("set-cookie");
	            	if (cookie) {
	            		cookie += "; OPEN_STICKY_SESSION=1;"
                ...
                that.emit('finish', status, x.responseText, cookie); //add by zhh for support cookie
                that._cleanup(false);
                break;
            }
        }
...
SockJS.prototype._didClose = function(code, reason, force) {
    ...
    if (that.readyState !== SockJS.CONNECTING &&
        ...
        that.readyState !== SockJS.CLOSING) {
        	utils.log('socket INVALID_STATE_ERR: ' + that.readyState);
        	return;
    ...
    if (!utils.userSetCode(code) &&
       ...
       utils.log(close_event.toString());
    }
...
SockJS.prototype._try_next_protocol = function(close_event) {
    ...
    while(1) {
        ...
        if (!SockJS[protocol] ||
            ...
            if (that._options.info.cookie)
            	that._debug("need cookie:" + that._options.info.cookie);
...
SockJS.prototype._applyInfo = function(info, rtt, protocols_whitelist) {
    ...
    that._options.info.null_origin = false;//zhh !_document.domain;
...
BufferedSender.prototype.send_schedule = function() {
    ...
    if (that.send_buffer.length > 0) {
        ...
        that.send_stop = that.sender(that.trans_url, payload, that.ri._options.info, function(success, abort_reason) {
            that.send_stop = null;
            if (success === false) {
...
var createAjaxSender = function(AjaxObject) {
    return function(url, payload, opts, callback) {
...
XhrStreamingTransport.need_body = false;//true;
...
InfoReceiver.prototype.doXhr = function(base_url, AjaxObject) {
    ...
    xo.onfinish = function(status, text, cookie) {
        clearTimeout(tref);
        tref = null;
        if (status === 200) {
...
Polling.prototype._scheduleRecv = function() {
    var that = this;
    var poll = that.poll = new that.Receiver(that.recv_url, that.AjaxObject, that.ri._options.info);
    var msg_counter = 0;
...
var XhrReceiver = function(url, AjaxObject, opts) {
    var that = this;
    var buf_pos = 0;
